...
| M4 어깨 한 축에서 AS5600 기반 제한 폐루프 목표각 제어를 검증했다 | [M4 어깨 AS5600 폐루프 벤치 검증](2026-06-28-m4-shoulder-closed-loop.md): 230-245도 검증 범위, 무부하/23.10g 회귀, safety block 0건, 센서 freshness/자석 상태 기록. 이후 ROS zero `222.80 deg`, sign `+1`, 현재 자세 조건부 `122.08-301.02 deg` soft range를 적용했다 | M4 한 축, 제한 하중 조건이다. `122.08-301.02 deg`는 현재 elbow/wrist/gripper 자세 조건부이며 global hard-stop model이 아니다. 나머지 네 축 위치 피드백과 장기 진동/전압별 반복은 미검증이다 | Full-arm closed loop, encoder-grade feedback on all joints, production joint control |
...
| `ros2_control` controller/hardware-interface boundary를 dry-run과 read-only measured mode로 검증했다 | [`ros2_control` open-loop evidence](2026-06-16-ros2-control-open-loop.md), `ros2_ws/src/motionbrain_hardware_interface/`, `ros2_ws/src/motionbrain_ros2_control_mock/`, M4 `m4_state` read-only launch/tests | `transport_mode=dry_run`의 `write()`는 ESP32에 물리 POST를 보내지 않는다. `m4_state`는 command interface 없이 cached `/motionbrain/status_typed`에서 유효한 M4 state만 노출한다 | Physical ros2_control actuation, closed-loop ros2_control hardware interface, trajectory tracking on real joints |
...
- Physical `ros2_control`: 미완료. 현재 증거는 dry-run/mock/open-loop scaffold와
  M4 read-only measured state mode다.
