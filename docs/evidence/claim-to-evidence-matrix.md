...
[PORTFOLIO](../../PORTFOLIO.md) | [로보틱스 시스템 준비도](../../ROBOTICS_SYSTEM_READINESS.md)
...
| M4 어깨 한 축에서 AS5600 기반 제한 폐루프 목표각 제어를 검증했다 | [M4 어깨 AS5600 폐루프 벤치 검증](2026-06-28-m4-shoulder-closed-loop.md): 230-245도 범위, 무부하/23.10g 회귀, safety block 0건, 센서 freshness/자석 상태 기록 | M4 한 축, 현재 장착 오프셋, 좁은 각도 범위, 제한 하중 조건이다. 나머지 네 축 위치 피드백과 장기 진동/전압별 반복은 미검증이다 | Full-arm closed loop, encoder-grade feedback on all joints, production joint control |
...
| `ros2_control` controller/hardware-interface boundary를 dry-run으로 검증했다 | [`ros2_control` open-loop evidence](2026-06-16-ros2-control-open-loop.md), `ros2_ws/src/motionbrain_hardware_interface/`, `ros2_ws/src/motionbrain_ros2_control_mock/` | `transport_mode=dry_run`; `write()`는 ESP32에 물리 POST를 보내지 않는다. State mirror와 interface bring-up 증거다 | Physical ros2_control actuation, closed-loop ros2_control hardware interface, trajectory tracking on real joints |
| Pi dashboard/perception pipeline과 제한 known-object `cup` 확인 경로가 있다 | [Pi health](2026-06-16-pi-system-health.md), [runtime measurement](2026-06-17-runtime-measurements.md), `tools/motionbrain_dashboard.py`, `tools/motionbrain_perception_service.py`, [PORTFOLIO 객체 인식 현황](../../PORTFOLIO.md#객체-인식-현황) | ESP32-CAM QVGA/JPEG quality 10과 YOLOv5s 기반의 제한된 bench 경로다. 모델 weight는 repository에 없고, active target은 `cup` 중심이다 | Arbitrary object recognition, marker/object-based autonomous grasping, continuous visual servoing |
| Pi/systemd 운영 표면과 SSH/DNS recovery 절차가 있다 | [Pi health](2026-06-16-pi-system-health.md), [runtime measurement](2026-06-17-runtime-measurements.md), `deploy/systemd/`, `tools/raspi/`, `OPERATIONS.md` | Captured Pi runtime은 point-in-time이다. `vcgencmd get_throttled` 값 `0x50005`가 기록되어 전원/스로틀 이슈를 별도 caveat로 둬야 한다 | Production uptime, power-integrity closure, thermal/performance guarantee |
...
- Physical `ros2_control`: 미완료. 현재 증거는 dry-run/mock/open-loop scaffold다.
