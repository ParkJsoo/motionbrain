...
void MotionBrainWebServer::handleRoot() {
  ...
  server_.sendContent(".vision-feed { background: #05080d; border-right: 1px solid var(--line-soft); display: flex; align-items: center; justify-content: center; min-width: 0; }");
  ...
  server_.sendContent(".camera-actions { display: grid; grid-template-columns: 1fr 1fr; gap: 8px; }");
  ...
  server_.sendContent("let cameraReconnectTimer = null; let cameraRetryTimer = null; let cameraMode = 'stream';");
  ...
  server_.sendContent("function clearCameraTimers() { if (cameraReconnectTimer) { clearTimeout(cameraReconnectTimer); cameraReconnectTimer = null; } if (cameraRetryTimer) { clearTimeout(cameraRetryTimer); cameraRetryTimer = null; } }");
  ...
  server_.sendContent("function snapshotCamera() { cameraMode = 'snapshot'; cameraBaseUrl = normalizeCameraUrl(cameraBaseUrl); const input = document.getElementById('camera-url'); if (input) input.value = cameraBaseUrl; localStorage.setItem('motionbrainCameraUrl', cameraBaseUrl); const img = document.getElementById('camera-stream'); if (!img) return; clearCameraTimers(); setCameraStatus('SNAPSHOT', 'ok'); img.src = cameraPath('/capture?t=' + Date.now()); }");
  ...
  server_.sendContent("function initCameraFeed() { const input = document.getElementById('camera-url'); const img = document.getElementById('camera-stream'); cameraBaseUrl = normalizeCameraUrl(cameraBaseUrl); if (input) input.value = cameraBaseUrl; if (img) { img.onerror = function() { if (cameraMode === 'stream') { setCameraStatus('RETRY', 'hot'); retryCameraStream(); } else { setCameraStatus('OFFLINE', 'hot'); } }; img.onload = function() { setCameraStatus(cameraMode === 'snapshot' ? 'SNAPSHOT' : 'STREAM', 'ok'); }; } document.addEventListener('visibilitychange', function() { if (document.hidden) { stopCameraFeed('PAUSED'); } else if (cameraMode === 'stream') { startCameraStream(); } }); startCameraStream(); }");
  ...
  server_.sendContent("window.addEventListener(\"load\", function() { updateTokenStatus(); validateDefaultSpeed(); initCameraFeed(); for (let i = 1; i <= MOTOR_COUNT; i++) { initJoystick(i); } });");
