...
void MotionBrainWebServer::handleRoot() {
  ...
  server_.sendContent(".ops-grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 8px; }");
  server_.sendContent(".ops-tile { min-height: 64px; padding: 10px; border: 1px solid var(--line-soft); border-radius: 8px; background: var(--panel-3); display: flex; flex-direction: column; justify-content: space-between; gap: 7px; min-width: 0; }");
  server_.sendContent(".ops-label { color: var(--muted); font-size: 10px; font-weight: 900; text-transform: uppercase; letter-spacing: 0.9px; }");
  server_.sendContent(".ops-value { color: #dbeafe; font-size: 12px; font-weight: 900; overflow-wrap: anywhere; line-height: 1.25; }");
  server_.sendContent(".ops-value.ok { color: #86efac; } .ops-value.warn { color: #fcd34d; } .ops-value.hot { color: #fca5a5; }");
  ...
  server_.sendContent("@media (max-width: 720px) { body { padding: 10px; } .container { grid-template-columns: 1fr; gap: 10px; } .app-header { align-items: flex-start; flex-direction: column; } .header-pills { justify-content: flex-start; } .brand-title { font-size: 28px; } .status-strip, .ops-grid { grid-template-columns: 1fr; } .vision-layout { grid-template-columns: 1fr; } .vision-feed { border-right: none; border-bottom: 1px solid var(--line-soft); min-height: 220px; } .card-vision, .card-command, .card-light, .card-motor, .card-joint { grid-column: 1 / -1; } .button-container.active, .joystick-container.active, .card-joint { grid-template-columns: 1fr; } .motor-controls { align-items: stretch; } .motor-controls button { flex: 1 1 30%; } }");
  ...
  server_.sendContent("<div class=\"card card-routine\">");
  server_.sendContent("<div class=\"card-title\">Routine Readiness</div>");
  server_.sendContent("<div class=\"ops-grid\">");
  server_.sendContent("<div class=\"ops-tile\"><span class=\"ops-label\">Recovery</span><span class=\"ops-value\" id=\"ops-recovery\">-</span></div>");
  server_.sendContent("<div class=\"ops-tile\"><span class=\"ops-label\">Executor</span><span class=\"ops-value\" id=\"ops-executor\">-</span></div>");
  server_.sendContent("<div class=\"ops-tile\"><span class=\"ops-label\">Feedback</span><span class=\"ops-value\" id=\"ops-feedback\">-</span></div>");
  server_.sendContent("<div class=\"ops-tile\"><span class=\"ops-label\">Sensor</span><span class=\"ops-value\" id=\"ops-sensor\">-</span></div>");
  server_.sendContent("<div class=\"ops-tile\"><span class=\"ops-label\">Teleop</span><span class=\"ops-value\" id=\"ops-teleop\">-</span></div>");
  server_.sendContent("<div class=\"ops-tile\"><span class=\"ops-label\">Safety</span><span class=\"ops-value\" id=\"ops-safety\">-</span></div>");
  server_.sendContent("<div class=\"ops-tile\"><span class=\"ops-label\">Last Command</span><span class=\"ops-value\" id=\"ops-last-command\">-</span></div>");
  server_.sendContent("</div></div>");
  ...
  server_.sendContent("function setOps(id, value, tone) { const el = document.getElementById(id); if (!el) return; el.textContent = value; el.className = 'ops-value' + (tone ? ' ' + tone : ''); }");
  ...
  server_.sendContent("const CAMERA_CAPTURE_REFRESH_MS = 2500;");
  server_.sendContent("const CAMERA_CAPTURE_RETRY_MS = 2000;");
  ...
  server_.sendContent("const TRACKED_CAPTURE_RELEASE_MS = 200;");
  ...
  server_.sendContent("function scheduleCameraReconnect() { if (cameraMode !== 'capture' || document.hidden) return; if (cameraReconnectTimer) clearTimeout(cameraReconnectTimer); cameraReconnectTimer = setTimeout(function() { if (cameraMode === 'capture') startCameraStream(); }, CAMERA_CAPTURE_REFRESH_MS); }");
  server_.sendContent("function startCameraStream() { trackedUserRequested = false; cameraMode = 'capture'; cameraBaseUrl = normalizeCameraUrl(cameraBaseUrl); const input = document.getElementById('camera-url'); if (input) input.value = cameraBaseUrl; localStorage.setItem('motionbrainCameraUrl', cameraBaseUrl); if (document.hidden) { stopCameraFeed('PAUSED'); return; } const img = document.getElementById('camera-stream'); if (!img) return; clearCameraTimers(); setCameraStatus('LIVE CAPTURE', 'ok'); setVisionOverlay({ detected: false, reason: 'live_capture' }); img.src = cameraPath('/capture?t=' + Date.now()); scheduleCameraReconnect(); syncCameraFromVisionConfig(false).then(changed => { if (changed && cameraMode === 'capture' && !document.hidden) img.src = cameraPath('/capture?t=' + Date.now()); }); }");
  server_.sendContent("function retryCameraStream() { if (cameraMode !== 'capture' || document.hidden) return; if (cameraRetryTimer) clearTimeout(cameraRetryTimer); cameraRetryTimer = setTimeout(function() { if (cameraMode === 'capture') startCameraStream(); }, CAMERA_CAPTURE_RETRY_MS); }");
  ...
  server_.sendContent("function startTrackedCamera(userRequested) { const wasCapture = cameraMode === 'capture'; trackedUserRequested = !!userRequested || trackedUserRequested; cameraMode = 'tracked'; visionBaseUrl = normalizeVisionUrl(visionBaseUrl); const input = document.getElementById('vision-url'); if (input) input.value = visionBaseUrl; localStorage.setItem('motionbrainVisionUrl', visionBaseUrl); if (document.hidden) { stopCameraFeed('PAUSED'); return; } const img = document.getElementById('camera-stream'); clearCameraTimers(); trackedFailureCount = 0; if (wasCapture && img) { setCameraStatus('RELEASING', 'warn'); setVisionOverlay({ detected: false, reason: 'releasing' }); trackedWarmupTimer = setTimeout(function() { trackedWarmupTimer = null; beginTrackedCamera(); }, TRACKED_CAPTURE_RELEASE_MS); img.removeAttribute('src'); return; } beginTrackedCamera(); }");
  server_.sendContent("function initCameraFeed() { const input = document.getElementById('camera-url'); const visionInput = document.getElementById('vision-url'); const img = document.getElementById('camera-stream'); cameraBaseUrl = normalizeCameraUrl(cameraBaseUrl); visionBaseUrl = normalizeVisionUrl(visionBaseUrl); if (input) input.value = cameraBaseUrl; if (visionInput) visionInput.value = visionBaseUrl; if (img) { img.onerror = function() { if (cameraMode === 'tracked') { if (trackedWarmupTimer) return; trackedFailureCount++; const warming = trackedFailureCount <= 2; setCameraStatus(warming ? 'WARMING' : 'VISION API', warming ? 'warn' : 'hot'); setVisionOverlay({ detected: false, reason: warming ? 'warming' : 'vision_api' }); if (!trackedUserRequested && trackedFailureCount >= TRACKED_STARTUP_FAILURE_LIMIT) startCameraStream(); else scheduleTrackedFrame(Math.min(5000, CAMERA_CAPTURE_RETRY_MS + trackedFailureCount * 400)); } else if (cameraMode === 'capture') { setCameraStatus('CAM CHECK', 'hot'); syncCameraFromVisionConfig(true).then(changed => { if (changed && cameraMode === 'capture' && !document.hidden) startCameraStream(); else retryCameraStream(); }); scheduleCameraConfigSync(true); } else { setCameraStatus('OFFLINE', 'hot'); } }; img.onload = function() { if (cameraMode === 'tracked') { trackedFailureCount = 0; setCameraStatus('TRACKED', 'ok'); scheduleTrackedFrame(TRACKED_FRAME_MS); } else { setCameraStatus(cameraMode === 'snapshot' ? 'SNAPSHOT' : 'LIVE CAPTURE', 'ok'); } }; } document.addEventListener('visibilitychange', function() { if (document.hidden) { stopCameraFeed('PAUSED'); } else if (cameraMode === 'tracked') { startTrackedCamera(trackedUserRequested); } else if (cameraMode === 'capture') { startCameraStream(); } }); startTrackedCamera(false); }");
  ...
  server_.sendContent("function formatLastCommand(cmd) { if (!cmd || !cmd.seen) return 'none'; const age = typeof cmd.ageMs === 'number' ? ' ' + Math.round(cmd.ageMs / 1000) + 's' : ''; return (cmd.type || 'command') + ' ' + (cmd.success ? 'OK' : 'FAIL') + age; }");
  server_.sendContent("function updateOpsFromStatus(data) { const recovery = data.recovery || {}; setOps('ops-recovery', recovery.action || 'none', recovery.action && recovery.action !== 'none' ? 'warn' : 'ok'); setOps('ops-last-command', formatLastCommand(data.lastCommand), data.lastCommand && data.lastCommand.success === false ? 'hot' : ''); }");
  server_.sendContent("function updateRoutinePanel() { fetch('/routine', { cache: 'no-store' }).then(r => { if (!r.ok) throw new Error('HTTP ' + r.status); return r.json(); }).then(data => { const diag = data.diagnostics || {}; const sensor = diag.sensor || {}; const teleop = diag.teleop || {}; const safety = diag.safety || {}; const executor = data.executor || {}; const feedback = data.feedback || {}; const baseYaw = feedback.baseYaw || {}; const feedbackFault = baseYaw.fault || feedback.blockReason || 'unknown'; const feedbackTarget = feedback.selectedClosureTarget || 'feedback'; setOps('ops-executor', (executor.status && executor.status.state ? executor.status.state : 'idle') + (executor.queueApplyAllowed ? ' queue' : ' blocked'), executor.queueApplyAllowed ? 'warn' : 'ok'); setOps('ops-feedback', feedbackTarget + ' ' + (feedback.readyForRoutineExecution ? 'ready' : feedbackFault), feedback.readyForRoutineExecution ? 'ok' : 'warn'); setOps('ops-sensor', (sensor.connected ? 'live' : 'offline') + ' ' + (typeof sensor.ageMs === 'number' ? sensor.ageMs + 'ms' : ''), sensor.fresh ? 'ok' : 'hot'); setOps('ops-teleop', (teleop.connected ? (teleop.deadman ? 'deadman' : 'standby') : 'offline') + ' ' + (typeof teleop.ageMs === 'number' ? teleop.ageMs + 'ms' : ''), teleop.connected ? 'ok' : 'hot'); setOps('ops-safety', safety.motionBlocked ? (safety.blockReason || 'blocked') : (safety.faultLatched ? (safety.faultReason || 'fault') : 'clear'), safety.motionBlocked || safety.faultLatched ? 'hot' : 'ok'); if (data.recovery) setOps('ops-recovery', data.recovery.action || 'none', data.recovery.action && data.recovery.action !== 'none' ? 'warn' : 'ok'); if (data.lastCommand) setOps('ops-last-command', formatLastCommand(data.lastCommand), data.lastCommand.success === false ? 'hot' : ''); }).catch(err => { setOps('ops-executor', 'offline', 'hot'); setOps('ops-feedback', 'offline', 'hot'); console.error('Routine update error:', err); }); }");
  ...
  server_.sendContent("setInterval(refreshJoystickLeases, MANUAL_LEASE_REFRESH_MS); setInterval(updateStatus, 1000); setInterval(updateRoutinePanel, 2000); updateStatus(); updateRoutinePanel();");
  server_.sendContent("</script></body></html>");
  
  DebugLog::info("Web Server: HTML sent successfully (tracked capture mode)");
}
