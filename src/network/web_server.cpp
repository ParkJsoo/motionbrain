...
void MotionBrainWebServer::handleRoot() {
  ...
  server_.sendContent("function updateRoutinePanel() { fetch('/routine', { cache: 'no-store' }).then(r => { if (!r.ok) throw new Error('HTTP ' + r.status); return r.json(); }).then(data => { const diag = data.diagnostics || {}; const sensor = diag.sensor || {}; const teleop = diag.teleop || {}; const safety = diag.safety || {}; const executor = data.executor || {}; setOps('ops-executor', (executor.status && executor.status.state ? executor.status.state : 'idle') + (executor.queueApplyAllowed ? ' queue' : ' blocked'), executor.queueApplyAllowed ? 'warn' : 'ok'); setOps('ops-sensor', (sensor.connected ? 'live' : 'offline') + ' ' + (typeof sensor.ageMs === 'number' ? sensor.ageMs + 'ms' : ''), sensor.fresh ? 'ok' : 'hot'); setOps('ops-teleop', (teleop.connected ? (teleop.deadman ? 'deadman' : 'standby') : 'offline') + ' ' + (typeof teleop.ageMs === 'number' ? teleop.ageMs + 'ms' : ''), teleop.connected ? 'ok' : 'hot'); setOps('ops-safety', safety.motionBlocked ? (safety.blockReason || 'blocked') : (safety.faultLatched ? (safety.faultReason || 'fault') : 'clear'), safety.motionBlocked || safety.faultLatched ? 'hot' : 'ok'); if (data.recovery) setOps('ops-recovery', data.recovery.action || 'none', data.recovery.action && data.recovery.action !== 'none' ? 'warn' : 'ok'); if (data.lastCommand) setOps('ops-last-command', formatLastCommand(data.lastCommand), data.lastCommand.success === false ? 'hot' : ''); }).catch(err => { setOps('ops-executor', 'offline', 'hot'); console.error('Routine update error:', err); }); }");
