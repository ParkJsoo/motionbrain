...
void MotionBrainWebServer::handleRoot() {
  ...
  server_.sendContent("const DEFAULT_VISION_URL = 'http://127.0.0.1:8765';");
  ...
  server_.sendContent("let cameraReconnectTimer = null; let cameraRetryTimer = null; let trackedFrameTimer = null; let visionPollTimer = null; let trackedWarmupTimer = null; let trackedFailureCount = 0; let cameraMode = 'tracked'; let trackedUserRequested = false; let lastVisionDetection = null;");
  ...
  server_.sendContent("function normalizeVisionUrl(value) { let url = (value || '').trim(); if (!url) url = DEFAULT_VISION_URL; if (!/^https?:\\/\\//i.test(url)) url = 'http://' + url; return url.replace(/\\/+$/, ''); }");
  ...
  server_.sendContent("function startTrackedCamera(userRequested) { const wasStream = cameraMode === 'stream'; trackedUserRequested = !!userRequested || trackedUserRequested; cameraMode = 'tracked'; visionBaseUrl = normalizeVisionUrl(visionBaseUrl); localStorage.setItem('motionbrainVisionUrl', visionBaseUrl); if (document.hidden) { stopCameraFeed('PAUSED'); return; } const img = document.getElementById('camera-stream'); clearCameraTimers(); trackedFailureCount = 0; if (wasStream && img) { setCameraStatus('RELEASING', 'warn'); setVisionOverlay({ detected: false, reason: 'releasing' }); trackedWarmupTimer = setTimeout(function() { trackedWarmupTimer = null; beginTrackedCamera(); }, TRACKED_STREAM_RELEASE_MS); img.removeAttribute('src'); return; } beginTrackedCamera(); }");
  server_.sendContent("function initCameraFeed() { const input = document.getElementById('camera-url'); const img = document.getElementById('camera-stream'); cameraBaseUrl = normalizeCameraUrl(cameraBaseUrl); visionBaseUrl = normalizeVisionUrl(visionBaseUrl); if (input) input.value = cameraBaseUrl; if (img) { img.onerror = function() { if (cameraMode === 'tracked') { if (trackedWarmupTimer) return; trackedFailureCount++; const warming = trackedFailureCount <= 2; setCameraStatus(warming ? 'WARMING' : 'VISION API', warming ? 'warn' : 'hot'); setVisionOverlay({ detected: false, reason: warming ? 'warming' : 'vision_api' }); if (!trackedUserRequested && trackedFailureCount >= TRACKED_STARTUP_FAILURE_LIMIT) startCameraStream(); else scheduleTrackedFrame(Math.min(5000, CAMERA_STREAM_RETRY_MS + trackedFailureCount * 400)); } else if (cameraMode === 'stream') { setCameraStatus('RETRY', 'hot'); retryCameraStream(); } else { setCameraStatus('OFFLINE', 'hot'); } }; img.onload = function() { if (cameraMode === 'tracked') { trackedFailureCount = 0; setCameraStatus('TRACKED', 'ok'); scheduleTrackedFrame(TRACKED_FRAME_MS); } else { setCameraStatus(cameraMode === 'snapshot' ? 'SNAPSHOT' : 'RAW STREAM', 'ok'); } }; } document.addEventListener('visibilitychange', function() { if (document.hidden) { stopCameraFeed('PAUSED'); } else if (cameraMode === 'tracked') { startTrackedCamera(trackedUserRequested); } else if (cameraMode === 'stream') { startCameraStream(); } }); startTrackedCamera(false); }");
