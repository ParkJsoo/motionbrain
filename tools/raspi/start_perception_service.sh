...
PERCEPTION_MAX_CAMERA_TIMEOUT="${MOTIONBRAIN_PERCEPTION_MAX_CAMERA_TIMEOUT:-5.0}"
PERCEPTION_STALE_SECONDS="${MOTIONBRAIN_PERCEPTION_STALE_SECONDS:-12.0}"
PERCEPTION_MIN_STALE_SECONDS="${MOTIONBRAIN_PERCEPTION_MIN_STALE_SECONDS:-12.0}"
...
if [[ "${MOTIONBRAIN_ALLOW_SHORT_CAMERA_STALE:-0}" != "1" ]]; then
  PERCEPTION_STALE_SECONDS="$(
    "${DISCOVERY_PYTHON}" -c '
import sys

stale = float(sys.argv[1])
minimum = float(sys.argv[2])
print(max(stale, minimum))
' "${PERCEPTION_STALE_SECONDS}" "${PERCEPTION_MIN_STALE_SECONDS}"
  )"
fi
...
args=(
  ...
  --stale-seconds "${PERCEPTION_STALE_SECONDS}"
